...
def writing_prompt(one: Dispute, found: Gathered, exhibits: list) -> str:
    ...
    return (
        ...
        "Never leave two different counts of the same thing standing in the "
        "same document. If the order says 25 leads and the sheet holds 35, "
        "say which is which and that more were delivered than were bought - "
        "an unexplained pair of numbers reads as carelessness in a document "
        "whose whole claim is that our records are exact. "
        "Put words in the cardholder's mouth only where the evidence shows "
        "the cardholder wrote them. A name sitting in a delivered row is our "
        "data, not their handwriting, and one misattributed line lets them "
        "contradict the paragraph it is in and take the rest of it down with "
        "it. Where you cannot tell which is which, quote only what is "
        "unmistakably theirs - 'my daughter', 'Beneficiary' - and leave the "
        "rest out. "
        "Say what our records contain, never how much of it there is. \"Our "
        "record of that delivery is the word delivered alongside the sheet "
        "link\" tells the acquirer our record is thin, which is an opinion "
        "rather than evidence and not one that helps. "
